...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function evaluateInterview(payload) {
        ...
        if (!response.ok) {
            throw new Error(result.message || "Не удалось оценить интервью.");
        }
    ...
    form.addEventListener("submit", async function (event) {
        ...
        setSuccess("Оцениваем интервью и сохраняем итоговый отчёт...");
        ...
        try {
            ...
            const scoreText = typeof result.score_total === "number"
                ? ` Балл: ${result.score_total}.`
                : "";

            const thresholdText = result.threshold_result
                ? ` Статус: ${result.threshold_result}.`
                : "";

            const recommendationText = result.recommendation
                ? ` Рекомендация: ${result.recommendation}.`
                : "";

            setSuccess(
                `Интервью завершено. Отчёт сохранён: ${result.filename}.${scoreText}${thresholdText}${recommendationText}`
            );
            ...
            setError(error.message || "Ошибка при оценке интервью.");
